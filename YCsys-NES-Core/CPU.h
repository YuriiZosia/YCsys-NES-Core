...
public:
    ...
    enum FLAGS6502 {
        ...
        Z = (1 << 1),   // Zero (Нуль). hex = 0x02, dec = 2
        I = (1 << 2),   // Disable Interrupts (Вимкнення переривань). hex = 0x04, dec = 4
        D = (1 << 3),   // Decimal Mode (Десятковий режим - в NES не використовується, але є в процесорі). hex = 0x08, dec = 8
		B = (1 << 4),   // Break (Програмне переривання). hex = 0x10, dec = 16
		U = (1 << 5),   // Unused (Не використовується, завжди 1). hex = 0x20, dec = 32
		V = (1 << 6),   // Overflow (Переповнення). hex = 0x40, dec = 64
        N = (1 << 7)    // Negative (Від'ємне число). hex = 0x80, dec = 128
    };
